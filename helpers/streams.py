#credit: http://git.zx2c4.com/zmusic-ng/tree/backend/zmusic/streams.py
...
def send_file_partial(path, mimetype=None, attachment_filename=None, headers=Headers()):
	headers.add('Content-Disposition', 'attachment', filename=attachment_filename)
	if current_app is not None and current_app.config.get('ACCEL_MUSIC_PREFIX'):
		headers.add('X-Accel-Redirect', os.path.join(current_app.config.get('ACCEL_MUSIC_PREFIX'), os.path.relpath(path, current_app.config.get('MUSIC_PATH'))))
